...
(defun write-inferred-superclasses-2 (&optional (kb (load-kb-jena "obi:newids;obidi.owl"))
				    (path "obi:newids;superclasses.owl"))
  (if (check kb)
      (with-ontology inferred (:base (format nil "http://purl.obofoundry.org/obo/obi/~a.~a"
					     (pathname-name (translate-logical-pathname path))
	  ...
	  ((loop for (sub super) in
		(set-difference (loop for class in (descendants !owl:Thing kb)
	      append
		(loop for super in (parents class kb)
		   collect (list class super)))
			   (sparql '(:select (?sub ?super) (:distinct t) (?sub !rdfs:subClassOf ?super)) :use-reasoner :jena :kb kbsimple) :test 'equal) collect
