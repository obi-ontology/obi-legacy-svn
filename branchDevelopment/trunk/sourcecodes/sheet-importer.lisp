...
(defun do-import (path &optional obi-id)
  (let ((new (make-hash-table :test 'equalp))
	headers)
    (flet ((field (key list)
	     (nth (position key headers :test (lambda(what where)(search what where))) list)))
      ;; collect the lines split into fields
      (let ((lines 
	     (with-open-file (f path)
	       (setq headers (split-at-char (read-line stream nil :eof) #\tab))
