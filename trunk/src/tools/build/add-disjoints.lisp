...
(defun write-disjoints (&optional (kb (load-kb-jena :obi)) (path "obi:branches;disjoints.owl"))
  (loop 
	 ...
	 (loop with queue = (list !<http://www.ifomis.org/bfo/1.1#Entity>) 
	    ...
	    with placeholders = (placeholder-classes kb)
		 ...
		 (let ((obi-disjoints (set-difference
		   ...
		   (dolist (other other-children)
		     (dolist (obi obi-children)
		       (unless (or (not (member other placeholders))
				   (not (member obi defined-classes))
				   (not (member other defined-classes))
				   (not (member obi placeholders)))
