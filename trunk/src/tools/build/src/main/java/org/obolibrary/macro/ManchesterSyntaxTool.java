// This code is a small modification by James A. Overton <james@overton.ca>
// to the original file here:
// https://code.google.com/p/oboformat/source/browse/trunk/src/main/java/org/obolibrary/macro/ManchesterSyntaxTool.java
...
public class ManchesterSyntaxTool {
	...
	public OWLIndividual parseManchesterIndividualExpression(String expression) throws ParserException {

		ManchesterOWLSyntaxEditorParser parser = createParser(expression);
		OWLIndividual ce = parser.parseIndividual();
		return ce;
	}

	public OWLOntology parseOntology(OWLOntologyManager manager, String expression) throws ParserException {

            try {
		ManchesterOWLSyntaxEditorParser parser = createParser(expression);
                OWLOntology ontology = manager.createOntology(IRI.create("http://foo.com"));
	...
	static class AdvancedEntityChecker implements OWLEntityChecker {
		...
		public OWLAnnotationProperty getOWLAnnotationProperty(String name) {
			OWLAnnotationProperty owlAnnotationProperty = defaultInstance.getOWLAnnotationProperty(name);
			if (owlAnnotationProperty == null) {
				IRI iri = getIRI(name);
				if (iri != null) {
					owlAnnotationProperty = getOWLAnnotationProperty(iri);
				}
			}
