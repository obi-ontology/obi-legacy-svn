...
The easiest way to integrate an existing body of work is to rely on the \ac{OWL}~\cite{OWL} mechanism \emph{owl:imports}, which imports the external resource as a whole.
...
Popular OWL tools (\emph{e.g.}, Prot\'eg\'e~\cite{Protege} and Pellet~\cite{Sirin}) can neither load nor reason over very large ontologies such as the NCBI Taxonomy~\cite{NCBI} or the Foundational Model of Anatomy~\cite{FMA}, making direct \ac{OWL} imports of such resources impractical. 
...
We tried several modularization tools~\cite{Grau2, Jimenez,Seidenberg,Sirin}. 
...
One tool~\cite{Seidenberg} %RB:\todo{which?} %MC: Seidenberg - but the tools have no name, so it is difficult to refer to them.
...
When a term's changes meaning, the term is deprecated~\cite{GOGuide}.
...
These queries~\cite{OBIQueries} specify, for each source ontology, which extra elements about the term is to be extracted, such as the definition and preferred label, and how to map these into the corresponding OBI annotation properties. 
