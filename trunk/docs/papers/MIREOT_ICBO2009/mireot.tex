...
Second, the other ontologies used may be under active development and not aligned with the chosen development methodology. Importing -policy wise- heterogenous ontologies as a whole could lead to inconsistencies or unintended inferences.
In this paper we propose a set of guidelines that aid ontology developers to import required terms from an external artefact into an own ontology. 
We describe the guidelines, their implementation, present some examples of application, and oultine future work and extensions.}
...
\label{intro}
The Web Ontology Language (OWL \cite{RefWorks:1506}) provides an in built mechanism to import ontologies (\emph{owl:imports}). Through author experience, in building large integrative ontologiese such as the Ontology for Biomedical Investigations (OBI \ref{*****}), the \emph{owl:imports} mechanism has proved insufficient for particular development scenarios. 
In addition, limitations in the currently available tools and reasoners can also make the implementation of the \emph{owl:imports} mechanism problematic.
For example, most current OWL tools can neither load nor reason over very large ontologies, such as the NCBI Taxonomy \cite{RefWorks:1502} or the Foundational Model of Anatomy \cite{RefWorks:1558}, making direct OWL imports of such ontologies. 
...
Ontologies are typically a dynamic knowledge resource. This is evident within the bio-ontology domain, where the codification of the knowledge must keep apace with scientific knowledge discovery. The flux of knowledge within an ontology can also present issues when trying to import or integrate different resources. The individual resources may have been constructed using different desing principles, which may not align. These issues of dynamic development, and different design principles could lead to inconsistencies or unintended inferences, when integrated.

To address these issues, we have developed a set of guidelines for importing classes from numerous ontology resources..
...
MIREOT allows the import of external terms from ontologies not yet using BFO as an upper ontology, or using other owl flavours than owl-DL or even differrent syntax formats, e.g. OBO Format.
...
external ontology URI: %DS: This is the core of the paper, don't just mention it in brackets, add def and example?
external term URI: %DS: def and example?
OBI superclass: %DS: maby make it independent from OBI: own ontologies superclass
...
Once the minimum information has been collected (as mentioned this is a manual command line step at the moment, but could be made more user friendly by implementing it via a Protege plugin) and stored in the \emph{external.owl} file, we can use it in order to get additional elements programatically.
...
It is worth mentioning that we only map existing annotation properties into our own metadata: we don't create new ones (so we do not integrate all external metadata, but just whats required by the OBI agreed minimal metadata policy ( Add URL as long minimal metadata is not out), (\emph{e.g.}, curation status annotation property, definition editor or definition source).
...
This is generated via a script that puts the results of the queries into the \emph{externalDerived.owl} file. 
This file can be removed on a regular basis (e.g. for a slim release), and rebuild via script based on \emph{external.owl}, allowing us to keep imported information up-to-date.
