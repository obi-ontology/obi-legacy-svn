...
\usepackage{url}
\usepackage[multiple, para]{footmisc} %RB added this to get commas between multiple footnotes (e.g., after "`endpoint"', seems to work but throws error. shrug.
...
The second example presents a slightly more complicated challenge. %RB sory what second example, was this brought up before? mayeb "`A second example of using the NCBI taxonomy terms presents a slighly more complicated challenge?"'  -- confusing as written
