...
It is written in OWL DL, and built using the Basic Formal Ontology%\footnote{http://ifomis.org/bfo/} 
...
One alternative to the OWL built-in import mechanism would be to copy only parts of the external ontology into \emph{obi.owl}. Software that extracts a ``module''\cite{module} is in early development and we plan to try such strategies in the future. %helpful to define module here to help explain
...
A second example presents a slightly more complicated challenge. %RB sory what second example, was this brought up before? mayeb "`A second example of using the NCBI taxonomy terms presents a slighly more complicated challenge?"'  -- confusing as written
...
To achieve this we classify the hierarchy and add the inferred superclasses via script to our OWL file, thus allowing end users to have the fully inferred hierarchy %(now a directed acyclic graph) - OBI is always a DAG
...
OBI, augmented with these 'assumed individuals', becomes more difficult to reason with reliably - we have had problems with both Pellet and Fact++ and are at the moment communicating with the developers of those reasoners to determine the source of the problem. %Rb just saying you have "`problems"' isn't very informative. State what these are %MC: well if we knew it woudl already be progress :)
